...
/**
 * La clase Ghost4 implementa una hebra que permite el movimiento del
 * fantasma 4
 *
...
public class Ghost4 implements Runnable {
    ...
    /**
     * Representa el estado cuando el fantasma 4 ha sido comido por el Pacman.
     * Por lo tanto permite que el fantasma se convierta en dos ojos que vuelven
     * a la celda inicial.
     */
    private static boolean volver4 = false;
    /**
     ...
     *
     */
    public Ghost4(ComecocosFrame fr, int nivel) {
        frame = fr;
     ...
     * fantasma con las iniciales (x=14,y=13). Hasta entonces, el fantasma
    ...
    public void run() {
        try {
            while (continuar) {
                ...
                Thread.sleep(delay);
                if ((frame.getRejilla().getTipoCelda(12, 13) == frame.getRejilla().nada) && (!Figura_ghost4.isComestible()) && subir) {
                    frame.getFig_ghost4().mueve(Figura_ghost4.ARRIBA);
                } else {
                    subir = false;
                    if ((!Figura_ghost4.isComestible()) && (volver4 == false)) {
                        ...
                        direccion_anterior = Figura_ghost4.direccion;
                        k = 0;
                        if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.IZQUIERDA)) && (direccion_anterior != Figura_ghost4.DERECHA)) {
                            ...
                            direcciones[k] = Figura_ghost4.IZQUIERDA;
                            posibles[0] = true;
                        }
                        k = 1;
                        if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.DERECHA)) && (direccion_anterior != Figura_ghost4.IZQUIERDA)) {
                            dist[k] = frame.distancia(frame.getFigura().getXorigen(), frame.getFig_ghost4().getXOrigen() + 1, frame.getFigura().getYOrigen(), frame.getFig_ghost4().getYOrigen());
                            direcciones[k] = Figura_ghost4.DERECHA;
                            posibles[1] = true;
                        }
                        k = 2;
                        if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.ARRIBA)) && (direccion_anterior != Figura_ghost4.ABAJO)) {
                            ...
                            direcciones[k] = Figura_ghost4.ARRIBA;
                            posibles[2] = true;
                        }
                        k = 3;
                        if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.ABAJO)) && (direccion_anterior != Figura_ghost4.ARRIBA)) {
                            dist[k] = frame.distancia(frame.getFigura().getXorigen(), frame.getFig_ghost4().getXOrigen(), frame.getFigura().getYOrigen(), frame.getFig_ghost4().getYOrigen() + 1);
                            direcciones[k] = Figura_ghost4.ABAJO;
                            posibles[3] = true;
                        }
                        k = 0;
                        double auxi1;
                        ...
                        if (num_posibilidades == 1) {
                            Figura_ghost4.direccion = direcciones[var];
                            frame.getFig_ghost4().mueve(Figura_ghost4.direccion);
                            direccion_anterior = direcciones[var];
                        } else {
                            double prob = Math.random();
                            if ((prob <= 0.5)) {
                                Figura_ghost4.direccion = direcciones[0];
                                frame.getFig_ghost4().mueve(Figura_ghost4.direccion);
                                direccion_anterior = direcciones[0];
                            } else if (prob > 0.5) {
                                Figura_ghost4.direccion = direcciones[1];
                                frame.getFig_ghost4().mueve(Figura_ghost4.direccion);
                        ...
                        if (volver4 == true) {
                            if ((Figura_ghost4.xorigen == 11) && (Figura_ghost4.yorigen == 13)) {
                                volver4 = false;
                                subir = false;
                            ...
                            k = 0;
                            if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.IZQUIERDA)) && (direccion_anterior != Figura_ghost4.DERECHA)) {
                                dist[k] = frame.distancia(11, frame.getFig_ghost4().getXOrigen() - 1, 13, frame.getFig_ghost4().getYOrigen());
                                direcciones[k] = Figura_ghost4.IZQUIERDA;
                                posibles[0] = true;
                            }
                            k = 1;
                            ...
                            k = 2;
                            if ((!frame.getRejilla().seChocaFantasma4(frame.getFig_ghost4(), Figura_ghost4.ARRIBA)) && (direccion_anterior != Figura_ghost4.ABAJO)) {
                                dist[k] = frame.distancia(11, frame.getFig_ghost4().getXOrigen(), 13, frame.getFig_ghost4().getYOrigen() - 1);
                                direcciones[k] = Figura_ghost4.ARRIBA;
                                posibles[2] = true;
                            }
                            k = 3;
    ...
    /**
     * Metodo que se encarga de fijar el valor del dato miembro volver4 a true o
     * false según el parámetro que recibe como entrada.
     *
     * @param volver4 de tipo boolean.
     *
     */
    public static void setVolver4(boolean volver4) {
        Ghost4.volver4 = volver4;
    }
