...
/**
 * La clase Ghost1 implementa una hebra que permite el movimiento del fantasma 1
 *
...
public class Ghost1 implements Runnable {
    ...
    /**
     * Representa el estado cuando el fantasma 1 ha sido comido por el Pacman.
     * Por lo tanto permite que el fantasma se convierta en dos ojos que vuelven
     * a la celda inicial.
     */
    private static boolean volver1 = false;
    /**
     ...
     *
     */
    public Ghost1(ComecocosFrame fr, int nivel) {
        frame = fr;
     ...
     * fantasma con las iniciales (x=11,y=13). Hasta entonces, el fantasma
    ...
    public void run() {
        try {
            while (continuar) {
                ...
                Thread.sleep(delay);
                if ((frame.getRejilla().getTipoCelda(12, 13) == frame.getRejilla().nada) && (!Figura_ghost1.isComestible()) && subir) {
                    frame.getFig_ghost1().mueve(Figura_ghost1.ARRIBA);
                } else {
                    subir = false;
                    if ((!Figura_ghost1.isComestible()) && (volver1 == false)) {
                        ...
                        direccion_anterior = Figura_ghost1.direccion;
                        //miercoles 3 junio 12:30 REDES 5 G ingeniero de telefÃ³nica
                        k = 0;
                        if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.IZQUIERDA)) && (direccion_anterior != Figura_ghost1.DERECHA)) {
                            ...
                            direcciones[k] = Figura_ghost1.IZQUIERDA;
                            posibles[0] = true;
                        }
                        k = 1;

                        if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.DERECHA)) && (direccion_anterior != Figura_ghost1.IZQUIERDA)) {
                            dist[k] = frame.distancia(frame.getFigura().getXorigen(), frame.getFig_ghost1().getXOrigen() + 1, frame.getFigura().getYOrigen(), frame.getFig_ghost1().getYOrigen());
                            direcciones[k] = Figura_ghost1.DERECHA;
                            posibles[1] = true;
                        }
                        k = 2;
                        if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.ARRIBA)) && (direccion_anterior != Figura_ghost1.ABAJO)) {
                            ...
                            direcciones[k] = Figura_ghost1.ARRIBA;
                            posibles[2] = true;
                        }
                        k = 3;
                        if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.ABAJO)) && (direccion_anterior != Figura_ghost1.ARRIBA)) {
                            dist[k] = frame.distancia(frame.getFigura().getXorigen(), frame.getFig_ghost1().getXOrigen(), frame.getFigura().getYOrigen(), frame.getFig_ghost1().getYOrigen() + 1);
                            direcciones[k] = Figura_ghost1.ABAJO;
                            posibles[3] = true;
                        ...
                        if (num_posibilidades == 1) {
                            Figura_ghost1.direccion = direcciones[var];
                            frame.getFig_ghost1().mueve(Figura_ghost1.direccion);
                            direccion_anterior = direcciones[var];
                        } else {
                            double prob = Math.random();
                            if ((prob <= 0.5)) {
                                Figura_ghost1.direccion = direcciones[0];
                                frame.getFig_ghost1().mueve(Figura_ghost1.direccion);
                                direccion_anterior = direcciones[0];
                            } else if (prob > 0.5) {
                                Figura_ghost1.direccion = direcciones[1];
                                frame.getFig_ghost1().mueve(Figura_ghost1.direccion);
                        ...
                        if (volver1 == true) {
                            if ((Figura_ghost1.getXorigen() == 11) && (Figura_ghost1.getYorigen() == 13)) {
                                volver1 = false;
                                subir = false;
                            ...
                            k = 0;
                            if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.IZQUIERDA)) && (direccion_anterior != Figura_ghost1.DERECHA)) {
                                dist[k] = frame.distancia(11, frame.getFig_ghost1().getXOrigen() - 1, 13, frame.getFig_ghost1().getYOrigen());
                                direcciones[k] = Figura_ghost1.IZQUIERDA;
                                posibles[0] = true;
                            }
                            k = 1;
                            ...
                            k = 2;
                            if ((!frame.getRejilla().seChocaFantasma1(frame.getFig_ghost1(), Figura_ghost1.ARRIBA)) && (direccion_anterior != Figura_ghost1.ABAJO)) {
                                dist[k] = frame.distancia(11, frame.getFig_ghost1().getXOrigen(), 13, frame.getFig_ghost1().getYOrigen() - 1);
                                direcciones[k] = Figura_ghost1.ARRIBA;
                                posibles[2] = true;
                            }
                            k = 3;
    ...
    /**
     * Metodo que se encarga de fijar el valor del dato miembro volver1 a true o
     * false según el parámetro que recibe como entrada.
     *
     * @param volver1 de tipo boolean.
     *
     */
    public static void setVolver1(boolean volver1) {
        Ghost1.volver1 = volver1;
    }
